...
本页为辅助中文摘要。[英文主文档](TOOLCHAIN_DIRECTION.md)优先维护，面向国际合作；
新增设计决策先更新英文版本，译文差异据其修正。

方向由 Mingli Yuan 于 2026-09-16 提出；本阶段设计、实现和记录由 ChatGPT
（OpenAI）完成，通过其授权账号代理提交。账号与授权不表示个人技术审查或正确性保证。

仓库现已按用户指定发布到 `mountain/adva-machine`，`origin/main` 为机器工作的
发布目标。原始 `adva` 正接入固定版本的机器与库；重构必须保留依赖与证据链。
第一条验收路径对照历史原生算术证据，并检查解释器的返回、拒绝和暂停及 Rust 重放。
旧源码路径与历史契约继续保留，详见英文主文档的依赖连续性一节。
